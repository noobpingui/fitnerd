...
# /sdd-test: etapa `tests` (implementer scaffold + test-author + verifier red)
...
3. **Etapa `tests`:** Si `tasks.md` tiene tareas `(scaffold)`, delega primero en `implementer` con **modo `scaffold`** (ADR-0012). Después delega en `test-author`. Después delega en `verifier` con **modo `red`**. Si el red check da `FAIL`, devuelve el caso al `test-author` con `verify-report.md` (`iterations.tests`, máximo 3). Si da `BLOCKED` por entorno, pide al usuario que lo resuelva. Presenta el gate solo con el red check en `PASS`.
