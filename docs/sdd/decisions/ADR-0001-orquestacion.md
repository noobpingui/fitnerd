...
- `.claude/commands/` sigue soportado, pero la documentación recomienda las skills (https://code.claude.com/docs/en/skills.md). Los nombres con dos puntos (`/sdd:run`) solo se obtienen con subdirectorios de `.claude/commands/` o con plugins. En las skills de proyecto, el nombre del comando es siempre el nombre del directorio (verificado en la tabla "How a skill gets its command name"). En la Fase 1 se había asumido lo contrario; se corrigió en la Fase 4.
...
- **Formato de los comandos:** skills en `.claude/skills/sdd-<etapa>/SKILL.md`, que se invocan como `/sdd-<etapa>` (`/sdd-run`, `/sdd-spec`, …). El protocolo compartido vive en `.claude/sdd/`. Decisión del usuario en la Fase 4: se eligió el formato vigente (skills) por encima de los dos puntos.
